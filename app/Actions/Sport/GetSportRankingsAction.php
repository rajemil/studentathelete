...
class GetSportRankingsAction
{
    /**
     * Get ranked students and chart data for a sport.
     *
     * @param  Sport  $sport
     * @param  User  $actor
     * @return array{ranked: Collection, chart: array}
     */
    public function execute(Sport $sport, User $actor): array
    {
        $studentIds = $sport->students()
            ->where('role', 'student')
            ->where('users.organization_id', $actor->organization_id)
            ->pluck('users.id');
