<x-app-layout>
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            ...
            <form method="POST" action="{{ route('sports.destroy', $sport) }}" onsubmit="return confirm('Delete this sport? This will also remove assignments.');">
                @csrf
                @method('DELETE')
                <button type="submit" class="inline-flex items-center rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-2 text-sm font-semibold text-red-400 hover:bg-red-500/20 hover:text-red-300 transition">
                    Delete
                </button>
            </form>
        </div>
    ...
    <div class="py-10">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="dash-card rounded-3xl p-8">
                <form method="POST" action="{{ route('sports.update', $sport) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
