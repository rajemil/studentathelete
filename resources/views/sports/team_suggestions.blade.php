<x-app-layout>
    <x-slot name="header">
        <div class="flex items-start justify-between gap-4">
            ...
            @php($staffBackUrl = \App\Support\StaffNavContext::backUrl())
            @if($staffBackUrl)
                <a href="{{ $staffBackUrl }}" class="inline-flex items-center rounded-lg border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:hover:bg-gray-700">
                    {{ \App\Support\StaffNavContext::backLabel() }}
                </a>
            @elseif(! request()->boolean('modal'))
    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            <div class="rounded-xl bg-white dark:bg-gray-800 shadow p-6">
                <form method="POST" action="{{ route('sports.team_suggestions.generate', array_filter(['sport' => $sport, 'modal' => request()->boolean('modal') ? '1' : null, 'context' => request()->query('context')])) }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    @csrf
