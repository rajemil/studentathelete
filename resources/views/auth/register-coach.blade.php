<x-guest-layout>
    <form method="POST" action="{{ route('register.coach') }}" x-data="{ birthdate: '{{ old('birthdate', '') }}', calcAge() { if (!this.birthdate) return ''; const d = new Date(this.birthdate); if (String(d) === 'Invalid Date') return ''; const now = new Date(); let age = now.getFullYear() - d.getFullYear(); const m = now.getMonth() - d.getMonth(); if (m < 0 || (m === 0 && now.getDate() < d.getDate())) age--; return age < 0 ? '' : age; } }">
        @csrf

        <div class="mb-5">
            <h1 class="text-xl font-semibold text-gray-900 dark:text-gray-100">Coach registration</h1>
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Create a coach account with your background and expertise.</p>
        </div>
