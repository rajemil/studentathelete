<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-lg sm:text-xl font-semibold tracking-tight text-gray-900 dark:text-gray-100">Schedule Event</h2>
            <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">Schedule intramurals, PE days, UWeek, school sports programs, and other team events.</div>
        </div>
    </x-slot>
    ...
    <div class="max-w-3xl rounded-2xl border border-gray-200/60 dark:border-white/10 bg-white/80 dark:bg-gray-900/50 p-6 shadow-sm">
        <form method="POST" action="{{ route('staff.events.store') }}" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. Morning Conditioning, Weekly Strategy Meeting" class="mt-1 block w-full rounded-xl border-gray-200 dark:border-white/10 bg-white/90 dark:bg-gray-950/40 text-sm focus:border-orange-500 focus:ring-orange-500" />
                ...
                <div>
                    ...
                    <select name="event_type" required class="mt-1 block w-full rounded-xl border-gray-200 dark:border-white/10 bg-white/90 dark:bg-gray-950/40 text-sm focus:border-orange-500 focus:ring-orange-500">
                        @foreach(\App\Support\EventTypes::labels() as $k => $v)
                            <option value="{{ $k }}" @selected(old('event_type', 'training') === $k)>{{ $v }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <div class="mt-1 text-2xs text-gray-500 dark:text-gray-400">All student athletes in this team will be enrolled as participants.</div>
                ...
                <div>
                    ...
                    <input type="datetime-local" name="starts_at" value="{{ old('starts_at', now()->format('Y-m-d\TH:00')) }}" required class="mt-1 block w-full rounded-xl border-gray-200 dark:border-white/10 bg-white/90 dark:bg-gray-950/40 text-sm focus:border-orange-500 focus:ring-orange-500" />
                ...
                <div>
                    ...
                    <input type="datetime-local" name="ends_at" value="{{ old('ends_at', now()->addHour()->format('Y-m-d\TH:00')) }}" required class="mt-1 block w-full rounded-xl border-gray-200 dark:border-white/10 bg-white/90 dark:bg-gray-950/40 text-sm focus:border-orange-500 focus:ring-orange-500" />
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" name="location" value="{{ old('location') }}" placeholder="e.g. Field A, Main Gym, Conference Room 2" class="mt-1 block w-full rounded-xl border-gray-200 dark:border-white/10 bg-white/90 dark:bg-gray-950/40 text-sm focus:border-orange-500 focus:ring-orange-500" />
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="4" placeholder="Event details, uniform/equipment requirements, or preparation notes..." class="mt-1 block w-full rounded-xl border-gray-200 dark:border-white/10 bg-white/90 dark:bg-gray-950/40 text-sm focus:border-orange-500 focus:ring-orange-500">{{ old('description') }}</textarea>
